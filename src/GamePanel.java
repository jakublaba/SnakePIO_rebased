...
public class GamePanel extends JPanel implements MouseListener, MouseMotionListener, ActionListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < GameBoard.mySnake.bodySegments.size(); i++) {
            snakeSegmentImg = new Ellipse2D.Double(GameBoard.mySnake.bodySegments.get(i).getX() - gameSegmentSize / 2,
                    GameBoard.mySnake.bodySegments.get(i).getY() - gameSegmentSize / 2, gameSegmentSize, gameSegmentSize);
            g2d.fill(snakeSegmentImg);
        }

        Ellipse2D.Double foodImg = new Ellipse2D.Double(GameBoard.myFood.getX() - gameSegmentSize / 2,
                GameBoard.myFood.getY() - gameSegmentSize / 2, gameSegmentSize, gameSegmentSize);
    ...
    private void updateDirection() {
        double mouseVectorX = mousePosition.getX() - GameBoard.mySnake.bodySegments.get(0).getX();
        double mouseVectorY = mousePosition.getY() - GameBoard.mySnake.bodySegments.get(0).getY();
        ...
        if (GameBoard.mySnake.bodySegments.size() > 2) {
            double bodyVectorX = GameBoard.mySnake.bodySegments.get(2).getX() - GameBoard.mySnake.bodySegments.get(0).getX();
            double bodyVectorY = GameBoard.mySnake.bodySegments.get(2).getY() - GameBoard.mySnake.bodySegments.get(0).getY();
